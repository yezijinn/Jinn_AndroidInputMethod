...
internal object ConfigBackupManager {
    ...
    /** 读取头部允许的连续「空读」次数（`InputStream.read` 契约允许返回 0，见 [unlockLocked]） */
    ...
    class ExportOutcome(
        val file: File,
        /** 未导出的剪贴板条数（0 = 全部导出）；口径见 [collectClipboard]：条数上限/单条上限/字节预算三类 */
        val clipDropped: Int,
    )
    ...
    private fun collectClipboard(context: Context): ClipPayload {
        ...
        while (true) {
            ...
            // （nextOffset 是扫描过的原始行数口径，items 是解密成功的条数，两者不能混用）
            if (page.nextOffset <= offset) break
            for (item in page.items) {
                // 与导入侧同一口径的条数上限：导入端只收前 N 条，多带的会在那边被静默截断
                if (taken >= ConfigBackup.MAX_CLIP_IMPORT_ITEMS) {
                    dropped++
                    continue
                }
                // 与导入侧同一口径的单条上限：超限条目导入时必然被丢掉，带上它只会造成
        ...
        val ok = runCatching {
            context.contentResolver.openInputStream(uri)?.use fileStream@ { input ->
                ...
                while (read < head.size) {
                    ...
                    if (n < 0) break
                    // 契约允许 read 返回 0（非阻塞流/恶意 provider）：不处理就是空转死循环
                    if (n == 0) {
    ...
    fun import(
        ...
        if (includeDicts) {
            // 包里有词库条目、manifest 却没登记摘要 ⇒ 与其它节同口径：判包不完整。
    ...
    /** 与 [sectionOf] 同口径，但直接吃算好的字节数组（导出路径复用它，避免同一份内容被转换多次） */
