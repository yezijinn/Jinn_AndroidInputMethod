...
internal object ConfigCrypto {
     ...
     * 密码长度定义域（按汉字个数计）。
     *
     * 下限 4 是用户 2026-09-23 定的口径（6 个字嫌多）。4 个汉字的组合空间已远大于常见弱口令，
     ...
     * 一个 `iterations=1` 的包照样能解开，与「600k 迭代」的安全承诺口径不一。10 万是默认值的
    ...
    /** 头部读取允许的连续「空读」次数：`InputStream.read` 契约允许返回 0（见 [readFully]） */
