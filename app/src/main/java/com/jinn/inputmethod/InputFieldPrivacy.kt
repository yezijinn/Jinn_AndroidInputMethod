 ...
 *    的**标准方式**；拿不到 password 变体的 App 走这条）。
 *    注意：imeOptions 侧**没有**公开的「不要联想」常量（`IME_FLAG_NO_SUGGESTIONS` 是 @hide，
 *    公开 SDK 引用它会直接编译失败），所以这里只认这一个标志。
 *
 * 注意：只引用 [EditorInfo] 的编译期常量（static final int 会被编译器内联），
...
object InputFieldPrivacy {
     ...
     *
     * 注意：不能用 0 表示"未知" —— [EditorInfo.TYPE_NULL] 的值就是 0，
