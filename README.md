...
**拼音键盘（26 键）**
- 全拼 / 双拼 / 英文；双拼 7 套（自然码、小鹤、搜狗、微软、紫光、智能ABC、加加），键面自动提示韵母与 zh/ch/sh；设置页「拼音输入方案」切换，选中即全局生效
- 不完整拼音补全（`ni m` / `nim` →「你们」）、词库约束分词（`xuni` → `xu+ni` 虚拟）
- 候选栏智能预测（默认关）；中英一键切换；候选栏上行显示实际按键，双拼下不会看着像没反应
- 圆角 / 间隙 / 透明度在设置页「键盘按钮调整」调（0~24dp / 0~8dp / 0~100%），松手即时生效
- 键盘皮肤：默认 / 磨砂 / 彩虹 / 极光 / 复古机械，纯代码绘制零位图，与上面三项独立
- 模糊音容错（默认关）：11 组口音等价类（zh⇄z、n⇄l、en⇄eng 等）按需勾选，命中不到时把该音的其他读法补在原有候选之后
