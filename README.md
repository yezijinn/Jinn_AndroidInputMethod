...
> ### 关于「语音听写」的说明
>
> 语音听写是**可选**功能，不开箱即用：识别**完全在服务端完成**，Android 端不打包任何模型。
> 服务端需要你自己在一台**家用 NAS** 上部署 [CapsWriter Offline](https://github.com/HaujetZhao/CapsWriter-Offline)
...
- **拼音键盘（26 键）**：全拼 / 双拼 / 英文；双拼共 **7 套方案**（自然码、小鹤、搜狗、微软、紫光、智能ABC、加加），
  键面自动提示韵母与 zh/ch/sh。输入方案在设置页「输入方案」里选（**全拼 + 7 套双拼**，选中即全局生效）。
