...
- **分类词库**：长词包（约 18.6 万词条）、腾讯大词库（约 97 万词条）可在设置页按需下载；
  只在**空闲时后台加载**（息屏 / 键盘闲置），**不影响打字**；首次构建索引后落盘，
  之后启动直接复用（约 0.05 秒）；增删后自动重启输入法生效。
- **用户词频学习**：记住你实际选过的候选并排到前面（对齐 librime 用户词典的加权与衰减），
  只学明确选择、数据只存本机不上传，可在设置页随时关闭。
