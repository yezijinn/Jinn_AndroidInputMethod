...
**拼音键盘（26 键）**
- 全拼 / 双拼 / 英文；双拼 7 套（自然码、小鹤、搜狗、微软、紫光、智能ABC、加加），键面自动提示韵母与 zh/ch/sh（设置页「键面韵母提示」可关，关闭后键面只显示字母）；设置页「拼音输入方案」切换，选中即全局生效
- 不完整拼音补全（`ni m` / `nim` →「你们」）、词库约束分词（`xuni` → `xu+ni` 虚拟）
- 候选栏：拼音条横排（显示按下的字母，双拼可切换为声韵全拼，如 `vsgo` → `zhongguo`），候选在其下；可选单行 / 双行，双行时拼音条夹在两排候选之间；候选栏追加候选词（默认关）；中英一键切换
