...
- 拼音键盘（26 键）：全拼 / 双拼 / 英文；双拼共 7 套方案（自然码、小鹤、搜狗、微软、紫光、智能ABC、加加），
  ...
  - 同一页可选**键盘皮肤**：默认 / 磨砂 / 彩虹 / 极光 / 复古机械 —— 只改键盘配色与质感
    （渐变、描边、键帽厚度），与圆角 / 间隙 / 透明度各自独立，点选即时生效；全部由代码绘制，不含图片资源。
...
- 配置备份（设置页「配置导出 / 导入」）：把全部设置项、用户词频（可选剪贴板历史与已下载词库）导出成一个
  `.jinn` 加密包，换设备时导入即可还原。整包 AES-256-GCM 加密，密码只用中文汉字、本机不保存；
  导入可选「覆盖还原」（设置与词频按备份写回）或「仅并入数据」（只合并词频与剪贴板，不动本机设置）。
  包内数据都可在导入前预览（备份时间 / 来源设备 / 各节条数）。
...
| `KeyAppearanceActivity` | 键盘外观页：26 键区圆角 / 间隙 + 透明度滑杆 + 键盘皮肤选择器 |
| `KeyboardSkin` | 键盘皮肤：色值与质感参数 / 5 套皮肤 / 解析与校验纯函数（零位图） |
| `SymbolOrderActivity` | 符号分组排序页：↑↓ 调整顺序 / 恢复默认 |
| `FavoriteSymbolsActivity` | 「收藏」符号编辑页：按页增删自定义符号 |
| `AsrClient`  | WebSocket 客户端：音频流式上传、断线指数退避重连（仅语音路径使用）|
| `ConfigBackup` | 备份包格式：prefs / 词频 / 剪贴板 / 词库的编解码与合并语义（纯函数，可 JVM 单测） |
| `ConfigBackupManager` | 备份包打包 / 解锁 / 清单 / 导入：各节上限、摘要校验、临时件清理 |
| `ConfigCrypto` | 备份包加解密：PBKDF2-HMAC-SHA256(600k) + AES-256-GCM（magic / 版本 / 迭代数护栏） |
...
服务端为部署在家用 NAS 上的 CapsWriter Offline（Docker），端口 `6016`。
...
+ 一个 `data=""` 的 `is_final=true` 收尾包组成；服务端返回整段累积文本，
客户端整体覆盖显示，绝不自行拼接。
...
### 签名说明
开源仓库不包含签名密钥与密码，且禁止把密钥库或口令文件放进仓库目录（否则一旦打包/备份整个项目即随之外泄）。如需签名构建：

1. 在仓库之外生成密钥库，例如 `<凭据目录>/com.jinn.inputmethod/release.jks`：
